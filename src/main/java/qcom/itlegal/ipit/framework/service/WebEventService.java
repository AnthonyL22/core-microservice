...
import qcom.itlegal.ipit.framework.siteminder.Authenticator;
import qcom.itlegal.ipit.framework.siteminder.BasicClientCookieMixIn;
import qcom.itlegal.ipit.framework.util.DateUtils;
import qcom.itlegal.ipit.framework.util.DebuggingUtils;
...
import static com.qualcomm.qssert.QssertService.*;
...
public class WebEventService extends WebEventController {
    ...
    private QualcommWebDriver qualcommWebDriver;
    private CloseableHttpClient qualcommHttpClient;
    ...
    public WebEventService(QualcommWebDriver driver) {
        this.qualcommWebDriver = driver;
    }
...
//            }
    ...
    private void addCookiesToDriver(final List<Cookie> cookies, final String siteMinderUrl) {
        ...
        try {
            if (cookies.size() > 0) {
                ...
                qualcommWebDriver.get(StringUtils.appendIfMissing(getUrl(), "/", "/") + siteMinderUrl);
                for (Cookie c : cookies) {
                    ...
                    qualcommWebDriver.manage().addCookie(cookie);
                }
            }
    ...
    protected void addCookiesToHttpClient(final List<Cookie> cookies) {
        ...
        if (cookies.size() > 0) {
            ...
            try {
                ...
                qualcommHttpClient = buildCookieBasedHttpClient(cookieStore, sslConnectionSocketFactory);
                ...
                loginResponse = qualcommHttpClient.execute(loginPost);
    ...
    public WebElement findWebElement(final String elementIdentifier) {
        ...
        if (elementIdentifier.matches(REGEX_XPATH_FINDER)) {
            try {
                WebElement webElement = this.qualcommWebDriver.findElementByXPath(elementIdentifier);
                if (webElement != null) {
                    return webElement;
                }
            } catch (Exception e) {
    ...
    public WebElement seleniumFindElementByXPath(List<String> xpathSearchList) {
        try {
            for (String xpathSearch : xpathSearchList) {
                ...
                try {
                    webElement = this.qualcommWebDriver.findElementByXPath(xpathSearch);
    ...
    public WebElement findElementByXPath(List<String> xpathSearchList) {
        ...
        try {
            ...
            String scrubbedBodyHtml = StringUtils.trim(StringUtils.substringBetween(this.qualcommWebDriver.getPageSource(), "</head>", "</html>"));
            ...
            for (String xpathSearch : xpathSearchList) {
                if ((boolean) xpath.evaluate(xpathSearch, document, XPathConstants.BOOLEAN)) {
                    return this.qualcommWebDriver.findElementByXPath(xpathSearch);
                }
            }
    ...
    public void hoverContains(final String elementIdentifier, final String expectedText, final boolean textExists) {
        Actions hoverAction = new Actions(qualcommWebDriver);
        ...
        if (textExists) {
            ...
            assertTrue("Verify hoverContains() exists text='%s'%s", pageContainsText(expectedText), expectedText, DebuggingUtils.getDebugInfo(this.qualcommWebDriver));
        } else {
            assertFalse("Verify hoverContains() exists text='%s'%s", pageContainsText(expectedText), expectedText, DebuggingUtils.getDebugInfo(this.qualcommWebDriver));
        }
    }
    ...
    public void tableRowCount(final String elementIdentifier, final int expectedRowCount) {
        ...
        try {
            ...
            DebuggingUtils.takeScreenShot(this.qualcommWebDriver);
    ...
    private void constructUrlWithFullHostFromCurrentUrl(String url) {
        ...
        Matcher m = URL_REGEX.matcher(this.qualcommWebDriver.getCurrentUrl());
        m.find();
    ...
    private void constructUrlWithQueryParameter(String url) {
        String currentUrl = this.qualcommWebDriver.getCurrentUrl();
    ...
    public long redirectToUrl(String url) {
        ...
        try {
            if (StringUtils.endsWith(qualcommWebDriver.getCurrentUrl(), getSiteMinderRedirectUrl())) {
                setUrl(url);
            ...
            sw.start();
            qualcommWebDriver.get(getUrl());
            //waitForBrowserToLoad();
            sw.stop();

            if (!StringUtils.equalsIgnoreCase(qualcommWebDriver.getCapabilities().getBrowserName(), "android")) {
                qualcommWebDriver.manage().window().maximize();
            }
    ...
    public void elementTextExists(final String expectedText, final boolean textExists) {
        if (textExists) {
            assertTrue("Verify elementTextExists() exists text='%s'%s", pageContainsText(expectedText), expectedText, DebuggingUtils.getDebugInfo(this.qualcommWebDriver));
        } else {
            assertFalse("Verify elementTextExists() exists text='%s'%s", pageContainsText(expectedText), expectedText, DebuggingUtils.getDebugInfo(this.qualcommWebDriver));
        }
    }
    ...
    public void pageTitleEquals(final String expectedPageTitle) {
        assertEquals("Verify Page Title", this.qualcommWebDriver.getTitle(), expectedPageTitle);
    }
    ...
    public Object executeJavascript(String javaScript) {
        try {
            return this.qualcommWebDriver.executeScript(javaScript);
        } catch (Exception e) {
            e.getMessage();
        }
        return null;
    ...
    public void alertTextEquals(final String expectedText, final String buttonTextToClick, final boolean expectedToExist) {
        try {
            ...
            WebDriverWait wait = new WebDriverWait(qualcommWebDriver, 2);
            wait.until(ExpectedConditions.alertIsPresent());
            Alert alert = qualcommWebDriver.switchTo().alert();
    ...
    private boolean pageContainsText(final String textToFind) {
        return this.qualcommWebDriver.getPageSource().contains(textToFind);
    }
    ...
    public void waitForElementToDisappear(final String elementIdentifier) {
        try {
            (new WebDriverWait(this.qualcommWebDriver, timeOutInSeconds, sleepInMillis)).until(new ExpectedCondition<Boolean>() {
    ...
    public void waitForElementToDisplayContainingText(final String elementIdentifier, final String textToWaitToDisplay) {
        try {
            ...
            (new WebDriverWait(this.qualcommWebDriver, timeOutInSeconds, sleepInMillis))
    ...
    public void waitForBrowserToLoad() {
        ...
        try {
            (new WebDriverWait(this.qualcommWebDriver, pageTimeoutInSeconds, sleepInMillis)).until(new ExpectedCondition<Boolean>() {
                ...
                public Boolean apply(WebDriver d) {
                    ...
                    //noActiveRequests = Boolean.valueOf(qualcommWebDriver.executeScript(FrameworkConstants.JAVASCRIPT_NO_AJAX_REQUESTS).toString());
    ...
    public void refreshBrowser() {
        qualcommWebDriver.navigate().refresh();
    }

    public void setQualcommWebDriver(QualcommWebDriver qualcommWebDriver) {
        this.qualcommWebDriver = qualcommWebDriver;
    }
    ...
    public void deleteCookie(final String cookieIdentifier) {
        ...
        for (org.openqa.selenium.Cookie cookie : cookiesToDelete) {
            this.qualcommWebDriver.manage().deleteCookie(cookie);
    ...
    public void addCookie(final String cookieName, final String cookieValue, String cookieDomain, String cookiePath, Object cookieExpiryDateOffset, boolean secureCookie) {
        ...
        this.qualcommWebDriver.manage().addCookie(cookieToAdd);
    ...
    private Set<org.openqa.selenium.Cookie> findCookies(final String cookieIdentifier) {
        ...
        Set<org.openqa.selenium.Cookie> cookies = this.qualcommWebDriver.manage().getCookies();
    ...
    public QualcommWebDriver getQualcommWebDriver() {
        return this.qualcommWebDriver;
    }

    public CloseableHttpClient getQualcommHttpClient() {
        return this.qualcommHttpClient;
    }
