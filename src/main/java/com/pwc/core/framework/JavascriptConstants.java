...
public final class JavascriptConstants {
    ...
    // TEXT ENTRY Action
    public static final String GET_ELEMENT_TEXT_JAVASCRIPT =
                    "var element = document.evaluate(\"%s\", document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue; return element.innerText";
    ...
    public static final String SELECT_VALUE_BY_XPATH =
                    ENTER_VALUE_ELEMENT_BY_XPATH + " for(var i=0; i < element.options.length; i++){if(element.options[i].text === \"%s\") {element.selectedIndex = i; break;}};";
    public static final String SELECT_VALUE_BY_IDENTIFIER_IDENTIFIER_VALUE_EXT_COMBO =
                    "Ext.ComponentQuery.query('combo#%s')[0].setValue(Ext.ComponentQuery.query('combo#%s')[0].getStore().findRecord('name','%s'))";
    public static final String SELECT_VALUE_BY_IDENTIFIER_INDEX_IDENTIFIER_EXT_COMBO =
                    "Ext.ComponentQuery.query('combo#%s')[%s].setValue(Ext.ComponentQuery.query('combo#%s')[%s].getStore().findRecord('name','%s'))";

    // CLICK Actions
    public static final String CLICK_BY_ELEMENT = "arguments[0].click();";
    ...
    public static final String DOUBLE_CLICK_BY_XPATH = "var element = document.evaluate(\"%s\", document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue; var clickEvent = "
                    + "document.createEvent('MouseEvents'); clickEvent.initEvent('dblclick', true, true); element.dispatchEvent (clickEvent)";
    public static final String CLICK_ELEMENT_BY_ID = "document.getElementById('%s').click();";
    public static final String CLICK_BY_TAG_AND_VALUE =
                    "var elements = document.getElementsByTagName('%s'); " + "for (var i=0; i<elements.length; i++) " + "{if (elements[i].value == '%s') elements[i].click();}";
    public static final String CLICK_BY_TAG_NODE_VALUE =
    ...
    public static final String CLICK_BY_TAG_TYPE_AND_EQUALS_VALUE =
    ...
    public static final String CLICK_BY_TAG_TYPE_AND_CONTAINS_VALUE = "var elements = document.getElementsByTagName('%s'); " + "for (var i=0; i<elements.length; i++) "
                    + "{if (elements[i].firstChild.textContent.indexOf('%s') > -1) elements[i].click();}";
    public static final String CLICK_BY_CLASS_NAME_AND_CONTAINS_VALUE = "var elements = document.getElementsByClassName('%s'); " + "for (var i=0; i<elements.length; i++) "
                    + "{if (elements[i].firstChild.textContent.indexOf('%s') > -1) elements[i].click();}";
    ...
    public static final String LIST_HTTP_RESOURCES = "var resourceList = []; var entries = window.performance.getEntriesByType('resource'); entries.forEach(function (resource) "
                    + "{ resourceList.push(resource.name);}); return(resourceList);";
