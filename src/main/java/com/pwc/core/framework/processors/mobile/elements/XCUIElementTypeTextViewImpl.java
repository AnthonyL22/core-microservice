...
public class XCUIElementTypeTextViewImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.TEXT_VIEW.type));
    }

    public void mobileAction(final WebElement mobileElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click TEXT VIEW %s", getElementText(mobileElement));
                mobileElement.click();
            } else {
                LOG(true, "Verify TEXT VIEW :: value='%s'", attributeValue);
                Assert.assertEquals(mobileElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed TEXT VIEW action due to exception=%s", e.getMessage());
        }
    }
