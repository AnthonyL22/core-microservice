...
public class XCUIElementTypeScrollViewImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.SCROLL_VIEW.type));
    }

    public void mobileAction(final WebElement mobileElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click SCROLL VIEW %s", getElementText(mobileElement));
                mobileElement.click();
            } else {
                LOG(true, "Verify SCROLL VIEW :: value='%s'", attributeValue);
                Assert.assertEquals(mobileElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed SCROLL VIEW action due to exception=%s", e.getMessage());
        }
    }
