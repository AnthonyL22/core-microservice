...
public class XCUIElementTypeRadioGroupImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.RADIO_GROUP.type));
    }

    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click RADIO GROUP %s", getElementText(WebElement));
                WebElement.click();
            } else {
                LOG(true, "Verify RADIO GROUP :: value='%s'", attributeValue);
                Assert.assertEquals(WebElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed RADIO GROUP action due to exception=%s", e.getMessage());
        }
    }
