...
public class XCUIElementTypeMenuItemImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.MENU_ITEM.type));
    }

    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click MENU ITEM %s", getElementText(WebElement));
                WebElement.click();
            } else {
                LOG(true, "Verify MENU ITEM :: value='%s'", attributeValue);
                Assert.assertEquals(WebElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed MENU ITEM action due to exception=%s", e.getMessage());
        }
    }
