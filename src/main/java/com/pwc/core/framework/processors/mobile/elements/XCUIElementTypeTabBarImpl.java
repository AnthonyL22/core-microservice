...
public class XCUIElementTypeTabBarImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.TAB_BAR.type));
    }

    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click TAB BAR %s", getElementText(WebElement));
                WebElement.click();
            } else {
                LOG(true, "Verify TAB BAR :: value='%s'", attributeValue);
                Assert.assertEquals(WebElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed TAB BAR action due to exception=%s", e.getMessage());
        }
    }
