...
public class XCUIElementTypeOutlineImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.OUTLINE.type));
    }

    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click OUTLINE %s", getElementText(WebElement));
                WebElement.click();
            } else {
                LOG(true, "Verify OUTLINE :: value='%s'", attributeValue);
                Assert.assertEquals(WebElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed OUTLINE action due to exception=%s", e.getMessage());
        }
    }
