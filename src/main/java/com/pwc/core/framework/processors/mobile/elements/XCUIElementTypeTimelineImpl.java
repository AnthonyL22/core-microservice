...
public class XCUIElementTypeTimelineImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.TIMELINE.type));
    }

    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click TIMELINE %s", getElementText(WebElement));
                WebElement.click();
            } else {
                LOG(true, "Verify TIMELINE :: value='%s'", attributeValue);
                Assert.assertEquals(WebElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed TIMELINE action due to exception=%s", e.getMessage());
        }
    }
