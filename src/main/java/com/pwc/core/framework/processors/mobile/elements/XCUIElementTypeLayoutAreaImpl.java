...
public class XCUIElementTypeLayoutAreaImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.LAYOUT_AREA.type));
    }

    public void mobileAction(final WebElement mobileElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click LAYOUT AREA %s", getElementText(mobileElement));
                mobileElement.click();
            } else {
                LOG(true, "Verify LAYOUT AREA :: value='%s'", attributeValue);
                Assert.assertEquals(mobileElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed LAYOUT AREA action due to exception=%s", e.getMessage());
        }
    }
