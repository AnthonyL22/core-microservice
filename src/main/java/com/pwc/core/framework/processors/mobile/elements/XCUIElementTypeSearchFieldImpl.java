...
public class XCUIElementTypeSearchFieldImpl implements MicroserviceWebElementElement {
    ...
    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click SEARCH FIELD %s", getElementText(WebElement));
                WebElement.click();
            } else {
                LOG(true, "Verify SEARCH FIELD :: value='%s'", attributeValue);
                Assert.assertEquals(WebElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed SEARCH FIELD action due to exception=%s", e.getMessage());
        }
    }
