...
public class XCUIElementTypeValueIndicatorImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.VALUE_INDICATOR.type));
    }

    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click VALUE INDICATOR %s", getElementText(WebElement));
                WebElement.click();
            } else {
                LOG(true, "Verify VALUE INDICATOR :: value='%s'", attributeValue);
                Assert.assertEquals(WebElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed VALUE INDICATOR action due to exception=%s", e.getMessage());
        }
    }
