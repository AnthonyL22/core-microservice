...
public class XCUIElementTypePageIndicatorImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.PAGE_INDICATOR.type));
    }

    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click PAGE INDICATOR %s", getElementText(WebElement));
                WebElement.click();
            } else {
                LOG(true, "Verify PAGE INDICATOR :: value='%s'", attributeValue);
                Assert.assertEquals(WebElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed PAGE INDICATOR action due to exception=%s", e.getMessage());
        }
    }
