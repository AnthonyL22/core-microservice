...
public class XCUIElementTypeDockItemImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.DOCK_ITEM.type));
    }

    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click DOCK ITEM %s", getElementText(WebElement));
                WebElement.click();
            } else {
                LOG(true, "Verify DOCK ITEM :: value='%s'", attributeValue);
                Assert.assertEquals(WebElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed DOCK ITEM action due to exception=%s", e.getMessage());
        }
    }
