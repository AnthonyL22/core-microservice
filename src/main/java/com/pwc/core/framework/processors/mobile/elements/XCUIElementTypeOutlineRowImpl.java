...
public class XCUIElementTypeOutlineRowImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.OUTLINE_ROW.type));
    }

    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click OUTLINE ROW %s", getElementText(WebElement));
                WebElement.click();
            } else {
                LOG(true, "Verify OUTLINE ROW :: value='%s'", attributeValue);
                Assert.assertEquals(WebElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed OUTLINE ROW action due to exception=%s", e.getMessage());
        }
    }
