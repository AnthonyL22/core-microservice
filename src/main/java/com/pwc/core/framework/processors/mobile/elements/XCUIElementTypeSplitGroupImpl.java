...
public class XCUIElementTypeSplitGroupImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.SPLIT_GROUP.type));
    }

    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click SPLIT GROUP %s", getElementText(WebElement));
                WebElement.click();
            } else {
                LOG(true, "Verify SPLIT GROUP :: value='%s'", attributeValue);
                Assert.assertEquals(WebElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed SPLIT GROUP action due to exception=%s", e.getMessage());
        }
    }
