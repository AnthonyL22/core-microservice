...
public class XCUIElementTypeToolbarImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.TOOLBAR.type));
    }

    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click TOOLBAR %s", getElementText(WebElement));
                WebElement.click();
            } else {
                LOG(true, "Verify TOOLBAR :: value='%s'", attributeValue);
                Assert.assertEquals(WebElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed TOOLBAR action due to exception=%s", e.getMessage());
        }
    }
