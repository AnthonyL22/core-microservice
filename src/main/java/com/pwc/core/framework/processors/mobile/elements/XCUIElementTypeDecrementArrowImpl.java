...
public class XCUIElementTypeDecrementArrowImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.DECREMENT_ARROW.type));
    }

    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click DECREMENT ARROW %s", getElementText(WebElement));
                WebElement.click();
            } else {
                LOG(true, "Verify DECREMENT ARROW :: value='%s'", attributeValue);
                Assert.assertEquals(WebElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed DECREMENT ARROW action due to exception=%s", e.getMessage());
        }
    }
