...
public class XCUIElementTypeTableImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.TABLE.type));
    }

    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click TABLE %s", getElementText(WebElement));
                WebElement.click();
            } else {
                LOG(true, "Verify TABLE :: value='%s'", attributeValue);
                Assert.assertEquals(WebElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed TABLE action due to exception=%s", e.getMessage());
        }
    }
