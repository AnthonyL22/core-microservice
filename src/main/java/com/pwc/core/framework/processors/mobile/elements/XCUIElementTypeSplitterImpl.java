...
public class XCUIElementTypeSplitterImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.SPLITTER.type));
    }

    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click SPLITTER %s", getElementText(WebElement));
                WebElement.click();
            } else {
                LOG(true, "Verify SPLITTER :: value='%s'", attributeValue);
                Assert.assertEquals(WebElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed SPLITTER action due to exception=%s", e.getMessage());
        }
    }
