...
public class XCUIElementTypeRulerImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.RULER.type));
    }

    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click RULER %s", getElementText(WebElement));
                WebElement.click();
            } else {
                LOG(true, "Verify RULER :: value='%s'", attributeValue);
                Assert.assertEquals(WebElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed RULER action due to exception=%s", e.getMessage());
        }
    }
