...
public class XCUIElementTypeMapImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.MAP.type));
    }

    public void mobileAction(final WebElement mobileElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click MAP %s", getElementText(mobileElement));
                mobileElement.click();
            } else {
                LOG(true, "Verify MAP :: value='%s'", attributeValue);
                Assert.assertEquals(mobileElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed MAP action due to exception=%s", e.getMessage());
        }
    }
