...
public class XCUIElementTypeTableRowImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.TABLE_ROW.type));
    }

    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click TABLE ROW %s", getElementText(WebElement));
                WebElement.click();
            } else {
                LOG(true, "Verify TABLE ROW :: value='%s'", attributeValue);
                Assert.assertEquals(WebElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed TABLE ROW action due to exception=%s", e.getMessage());
        }
    }
