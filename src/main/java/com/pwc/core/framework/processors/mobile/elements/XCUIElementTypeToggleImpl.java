...
public class XCUIElementTypeToggleImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.TOGGLE.type));
    }

    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click TOGGLE %s", getElementText(WebElement));
                WebElement.click();
            } else {
                LOG(true, "Verify TOGGLE :: value='%s'", attributeValue);
                Assert.assertEquals(WebElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed TOGGLE action due to exception=%s", e.getMessage());
        }
    }
