...
public class XCUIElementTypeCollectionViewImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.COLLECTION_VIEW.type);
    }

    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click COLLECTION VIEW %s", getElementText(WebElement));
                WebElement.click();
            } else {
                LOG(true, "Verify COLLECTION VIEW :: value='%s'", attributeValue);
                Assert.assertEquals(WebElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed COLLECTION VIEW action due to exception=%s", e.getMessage());
        }
    }
