...
public class XCUIElementTypeSliderImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.SLIDER.type));
    }

    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click SLIDER %s", getElementText(WebElement));
                WebElement.click();
            } else {
                LOG(true, "Verify SLIDER :: value='%s'", attributeValue);
                Assert.assertEquals(WebElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed SLIDER action due to exception=%s", e.getMessage());
        }
    }
