...
public class XCUIElementTypeHelpTagImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.HELP_TAG.type));
    }

    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click HELP TAG %s", getElementText(WebElement));
                WebElement.click();
            } else {
                LOG(true, "Verify HELP TAG :: value='%s'", attributeValue);
                Assert.assertEquals(WebElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed HELP TAG action due to exception=%s", e.getMessage());
        }
    }
