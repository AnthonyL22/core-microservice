...
public class XCUIElementTypeSegmentedControlImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.SEGMENTED_CONTROL.type));
    }

    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click SEGMENTED CONTROL %s", getElementText(WebElement));
                WebElement.click();
            } else {
