...
public class XCUIElementTypeMenuBarItemImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.MENU_BAR_ITEM.type));
    }

    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click MENU BAR ITEM %s", getElementText(WebElement));
                WebElement.click();
            } else {
                LOG(true, "Verify MENU BAR ITEM :: value='%s'", attributeValue);
                Assert.assertEquals(WebElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed MENU BAR ITEM action due to exception=%s", e.getMessage());
        }
    }
