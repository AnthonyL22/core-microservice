...
public class XCUIElementTypeMenuButtonImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.MENU_BUTTON.type));
    }

    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click MENU BUTTON %s", getElementText(WebElement));
                WebElement.click();
            } else {
                LOG(true, "Verify MENU BUTTON :: value='%s'", attributeValue);
                Assert.assertEquals(WebElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed MENU BUTTON action due to exception=%s", e.getMessage());
        }
    }
