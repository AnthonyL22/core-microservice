...
public class XCUIElementTypeDatePickerImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.DATE_PICKER.type);
    }

    public void mobileAction(final WebElement mobileElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click DATE PICKER %s", getElementText(mobileElement));
                mobileElement.click();
            } else {
                LOG(true, "Verify DATE PICKER :: value='%s'", attributeValue);
                Assert.assertEquals(mobileElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed DATE PICKER action due to exception=%s", e.getMessage());
        }
    }
