...
public class XCUIElementTypeGridImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.GRID.type));
    }

    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click GRID %s", getElementText(WebElement));
                WebElement.click();
            } else {
                LOG(true, "Verify GRID :: value='%s'", attributeValue);
                Assert.assertEquals(WebElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed GRID action due to exception=%s", e.getMessage());
        }
    }
