...
public class XCUIElementTypeActivityIndicatorImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.ACTIVITY_INDICATOR.type);
    }

    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click ACTIVITY INDICATOR %s", getElementText(WebElement));
                WebElement.click();
            } else {
