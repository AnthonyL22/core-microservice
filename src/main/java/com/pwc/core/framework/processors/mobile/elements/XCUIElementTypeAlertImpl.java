...
public class XCUIElementTypeAlertImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.ALERT.type);
    }

    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click ALERT %s", getElementText(WebElement));
                WebElement.click();
            } else {
                LOG(true, "Verify ALERT :: value='%s'", attributeValue);
                Assert.assertEquals(WebElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed ALERT action due to exception=%s", e.getMessage());
        }
    }
