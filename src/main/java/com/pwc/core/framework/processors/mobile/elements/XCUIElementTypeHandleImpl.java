...
public class XCUIElementTypeHandleImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.HANDLE.type));
    }

    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click HANDLE %s", getElementText(WebElement));
                WebElement.click();
            } else {
                LOG(true, "Verify HANDLE :: value='%s'", attributeValue);
                Assert.assertEquals(WebElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed HANDLE action due to exception=%s", e.getMessage());
        }
    }
