...
public class XCUIElementTypeSwitchImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.SWITCH.type));
    }

    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click SWITCH %s", getElementText(WebElement));
                WebElement.click();
            } else {
                LOG(true, "Verify SWITCH :: value='%s'", attributeValue);
                Assert.assertEquals(WebElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed SWITCH action due to exception=%s", e.getMessage());
        }
    }
