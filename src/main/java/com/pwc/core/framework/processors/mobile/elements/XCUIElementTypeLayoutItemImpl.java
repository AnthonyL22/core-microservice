...
public class XCUIElementTypeLayoutItemImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.LAYOUT_ITEM.type));
    }

    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click LAYOUT ITEM %s", getElementText(WebElement));
                WebElement.click();
            } else {
                LOG(true, "Verify LAYOUT ITEM :: value='%s'", attributeValue);
                Assert.assertEquals(WebElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed LAYOUT ITEM action due to exception=%s", e.getMessage());
        }
    }
