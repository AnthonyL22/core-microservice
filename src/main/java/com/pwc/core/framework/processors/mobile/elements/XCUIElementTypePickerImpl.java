...
public class XCUIElementTypePickerImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.PICKER.type));
    }

    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click PICKER %s", getElementText(WebElement));
                WebElement.click();
            } else {
                LOG(true, "Verify PICKER :: value='%s'", attributeValue);
                Assert.assertEquals(WebElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed PICKER action due to exception=%s", e.getMessage());
        }
    }
