...
public class XCUIElementTypeMatteImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.MATTE.type));
    }

    public void mobileAction(final WebElement mobileElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click MATTE %s", getElementText(mobileElement));
                mobileElement.click();
            } else {
                LOG(true, "Verify MATTE :: value='%s'", attributeValue);
                Assert.assertEquals(mobileElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed MATTE action due to exception=%s", e.getMessage());
        }
    }
