...
public class XCUIElementTypeMenuImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.MENU.type));
    }

    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click MENU %s", getElementText(WebElement));
                WebElement.click();
            } else {
                LOG(true, "Verify MENU :: value='%s'", attributeValue);
                Assert.assertEquals(WebElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed MENU action due to exception=%s", e.getMessage());
        }
    }
