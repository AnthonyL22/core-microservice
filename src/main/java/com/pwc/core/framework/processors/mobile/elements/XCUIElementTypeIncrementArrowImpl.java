...
public class XCUIElementTypeIncrementArrowImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.INCREMENT_ARROW.type));
    }

    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click INCREMENT ARROW %s", getElementText(WebElement));
                WebElement.click();
            } else {
                LOG(true, "Verify INCREMENT ARROW :: value='%s'", attributeValue);
                Assert.assertEquals(WebElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed INCREMENT ARROW action due to exception=%s", e.getMessage());
        }
    }
