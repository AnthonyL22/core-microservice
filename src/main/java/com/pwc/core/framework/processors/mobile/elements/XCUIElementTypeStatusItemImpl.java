...
public class XCUIElementTypeStatusItemImpl implements MicroserviceWebElementElement {
    ...
    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click STATUS ITEM %s", getElementText(WebElement));
                WebElement.click();
            } else {
                LOG(true, "Verify STATUS ITEM :: value='%s'", attributeValue);
                Assert.assertEquals(WebElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed STATUS ITEM action due to exception=%s", e.getMessage());
        }
    }
