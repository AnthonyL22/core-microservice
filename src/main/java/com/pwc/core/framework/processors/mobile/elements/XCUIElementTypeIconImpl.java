...
public class XCUIElementTypeIconImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.ICON.type));
    }

    public void mobileAction(final WebElement mobileElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click ICON %s", getElementText(mobileElement));
                mobileElement.click();
            } else {
                LOG(true, "Verify ICON :: value='%s'", attributeValue);
                Assert.assertEquals(mobileElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed ICON action due to exception=%s", e.getMessage());
        }
    }
