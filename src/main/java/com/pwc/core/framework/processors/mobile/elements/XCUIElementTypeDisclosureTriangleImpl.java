...
public class XCUIElementTypeDisclosureTriangleImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.DISCLOSURE_TRIANGLE.type));
    }

    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click DISCLOSURE TRIANGLE %s", getElementText(WebElement));
                WebElement.click();
            } else {
