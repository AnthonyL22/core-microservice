...
public class PDropDownElementImpl implements MicroserviceWebElement {
    ...
    public void webAction(final WebElement webElement, final Object attributeValue) {
        try {
            ...
            PDropDown pDropDown = new PDropDown(webElement);
            ...
            if (StringUtils.containsAny(String.valueOf(attributeValue), ",")) {
                ...
                IntStream.range(0, itemsToSelect.length).forEach(item -> pDropDown.selectByVisibleText(String.valueOf(item).trim()));
            } else {
                pDropDown.selectByVisibleText(String.valueOf(attributeValue));
            }
