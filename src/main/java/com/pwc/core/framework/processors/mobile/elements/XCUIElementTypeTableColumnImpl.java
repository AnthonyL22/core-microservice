...
public class XCUIElementTypeTableColumnImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.TABLE_COLUMN.type));
    }

    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click TABLE COLUMN %s", getElementText(WebElement));
                WebElement.click();
            } else {
