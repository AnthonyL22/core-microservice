...
public class XCUIElementTypeDrawerImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.DRAWER.type));
    }

    public void mobileAction(final WebElement mobileElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click DRAWER %s", getElementText(mobileElement));
                mobileElement.click();
            } else {
                LOG(true, "Verify DRAWER :: value='%s'", attributeValue);
                Assert.assertEquals(mobileElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed DRAWER action due to exception=%s", e.getMessage());
        }
    }
