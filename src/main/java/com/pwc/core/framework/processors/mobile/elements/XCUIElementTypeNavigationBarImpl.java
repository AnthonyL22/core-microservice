...
public class XCUIElementTypeNavigationBarImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.NAVIGATION_BAR.type));
    }

    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click NAVIGATION %s", getElementText(WebElement));
                WebElement.click();
            } else {
                LOG(true, "Verify NAVIGATION :: value='%s'", attributeValue);
                Assert.assertEquals(WebElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed NAVIGATION action due to exception=%s", e.getMessage());
        }
    }
