...
public class XCUIElementTypeLinkImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.LINK.type));
    }

    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click LINK %s", getElementText(WebElement));
                WebElement.click();
            } else {
                LOG(true, "Verify LINK :: value='%s'", attributeValue);
                Assert.assertEquals(WebElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed LINK action due to exception=%s", e.getMessage());
        }
    }
