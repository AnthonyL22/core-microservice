...
public class XCUIElementTypeRadioButtonImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.RADIO_BUTTON.type));
    }

    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click RADIO BUTTON %s", getElementText(WebElement));
                WebElement.click();
            } else {
                LOG(true, "Verify RADIO BUTTON :: value='%s'", attributeValue);
                Assert.assertEquals(WebElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed RADIO BUTTON action due to exception=%s", e.getMessage());
        }
    }
