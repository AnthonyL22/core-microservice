...
public class XCUIElementTypeApplicationImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.APPLICATION.type);
    }

    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click APPLICATION %s", getElementText(WebElement));
                WebElement.click();
            } else {
