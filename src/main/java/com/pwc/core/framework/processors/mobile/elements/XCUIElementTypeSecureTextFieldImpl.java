...
public class XCUIElementTypeSecureTextFieldImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.SECURE_TEXT_FIELD.type));
    }

    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click SECURE TEXT %s", getElementText(WebElement));
                WebElement.click();
            } else {
                LOG(true, "Verify SECURE TEXT :: value='%s'", attributeValue);
                Assert.assertEquals(WebElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed SECURE TEXT action due to exception=%s", e.getMessage());
        }
    }
