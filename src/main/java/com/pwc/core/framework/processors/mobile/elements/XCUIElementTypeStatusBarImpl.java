...
public class XCUIElementTypeStatusBarImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.STATUS_BAR.type));
    }

    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click STATUS BAR %s", getElementText(WebElement));
                WebElement.click();
            } else {
                LOG(true, "Verify STATUS BAR :: value='%s'", attributeValue);
                Assert.assertEquals(WebElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed STATUS BAR action due to exception=%s", e.getMessage());
        }
    }
