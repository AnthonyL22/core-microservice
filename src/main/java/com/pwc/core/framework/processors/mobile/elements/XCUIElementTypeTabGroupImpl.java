...
public class XCUIElementTypeTabGroupImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.TAB_GROUP.type));
    }

    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click TAB GROUP %s", getElementText(WebElement));
                WebElement.click();
            } else {
                LOG(true, "Verify TAB GROUP :: value='%s'", attributeValue);
                Assert.assertEquals(WebElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed TAB GROUP action due to exception=%s", e.getMessage());
        }
    }
