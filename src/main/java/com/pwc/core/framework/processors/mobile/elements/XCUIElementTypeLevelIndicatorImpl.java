...
public class XCUIElementTypeLevelIndicatorImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.LEVEL_INDICATOR.type));
    }

    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click LEVEL INDICATOR %s", getElementText(WebElement));
                WebElement.click();
            } else {
                LOG(true, "Verify LEVEL INDICATOR :: value='%s'", attributeValue);
                Assert.assertEquals(WebElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed LEVEL INDICATOR action due to exception=%s", e.getMessage());
        }
    }
