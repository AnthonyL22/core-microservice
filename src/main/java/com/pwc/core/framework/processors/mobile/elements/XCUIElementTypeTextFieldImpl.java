...
public class XCUIElementTypeTextFieldImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.TEXT_FIELD.type));
    }

    public void mobileAction(final WebElement mobileElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click TEXT FIELD %s", getElementText(mobileElement));
                mobileElement.click();
            } else {
                LOG(true, "Verify TEXT FIELD :: value='%s'", attributeValue);
                Assert.assertEquals(mobileElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed TEXT FIELD action due to exception=%s", e.getMessage());
        }
    }
