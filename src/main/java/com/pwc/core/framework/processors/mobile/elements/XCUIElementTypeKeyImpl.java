...
public class XCUIElementTypeKeyImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.KEY.type));
    }

    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click KEY %s", getElementText(WebElement));
                WebElement.click();
            } else {
                LOG(true, "Verify KEY :: value='%s'", attributeValue);
                Assert.assertEquals(WebElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed KEY action due to exception=%s", e.getMessage());
        }
    }
