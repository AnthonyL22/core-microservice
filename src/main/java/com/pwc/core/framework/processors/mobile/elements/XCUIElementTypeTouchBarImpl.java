...
public class XCUIElementTypeTouchBarImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.TOUCH_BAR.type));
    }

    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click TOUCH BAR %s", getElementText(WebElement));
                WebElement.click();
            } else {
                LOG(true, "Verify TOUCH BAR :: value='%s'", attributeValue);
                Assert.assertEquals(WebElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed TOUCH BAR action due to exception=%s", e.getMessage());
        }
    }
