...
public class XCUIElementTypeCheckBoxImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.CHECK_BOX.type);
    }

    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click CHECK BOX %s", getElementText(WebElement));
                WebElement.click();
            } else {
                LOG(true, "Verify CHECK BOX :: value='%s'", attributeValue);
                Assert.assertEquals(WebElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed CHECK BOX action due to exception=%s", e.getMessage());
        }
    }
