...
public class XCUIElementTypeKeyboardImpl implements MicroserviceWebElementElement {

    public static boolean applies(WebElement element) {
        return (StringUtils.equalsIgnoreCase(element.getTagName(), XCUIElementType.KEYBOARD.type));
    }

    public void mobileAction(final WebElement WebElement, final Object attributeValue) {
        try {
            if (attributeValue == null) {
                LOG(true, "Click KEYBOARD %s", getElementText(WebElement));
                WebElement.click();
            } else {
                LOG(true, "Verify KEYBOARD :: value='%s'", attributeValue);
                Assert.assertEquals(WebElement.getText(), attributeValue);
            }
        } catch (Exception e) {
            assertFail("Failed KEYBOARD action due to exception=%s", e.getMessage());
        }
    }
